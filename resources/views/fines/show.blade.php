                ...
                <div class="grid grid-cols-2 gap-6 mb-8">
                    <div class="border-b border-gray-200 pb-4">
                        ...
                        <p class="text-lg text-gray-800 mt-1">
                            <i class="fas fa-user text-blue-600"></i> {{ $fine->user->name }}
                        </p>
                        <p class="text-sm text-gray-600">{{ $fine->user->email }}</p>
                    </div>
                ...
                <div class="flex gap-3">
                    @if($fine->estado === 'pendiente')
                        ...
                        <form action="{{ route('fines.forgive', $fine->id) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition" onclick="return confirm('¿Condonar esta multa?')">
                        ...
                        @can('fines.editar')
                        <a href="{{ route('fines.edit', $fine->id) }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">
                            <i class="fas fa-edit"></i> Editar
                        </a>
                        @endcan
                    @endif

                    <a href="{{ route('fines.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition">
                    ...
                    <form action="{{ route('fines.destroy', $fine->id) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition" onclick="return confirm('¿Eliminar esta multa?')">
